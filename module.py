...
def cal_scale_zero_point(min_val, max_val, num_bits=8):
    qmin = 0.
    qmax = 2. ** num_bits - 1
...
class QParam(nn.Module):
    ...
    def update(self, tensor):
        ...
        self.scale, self.zero_point = cal_scale_zero_point(self.min, self.max, self.num_bits)

    def quantize_tensor(self, tensor):
        return quantize_tensor(tensor, self.scale, self.zero_point, num_bits=self.num_bits)

    def dequantize_tensor(self, tensor):
        return dequantize_tensor(tensor, self.scale, self.zero_point)
...
class QLinear(QModule):
    ...
    def __init__(self, fc_module, qi=True, qo=True, num_bits=8):
        ...
        self.register_buffer('M', M)  # 将M注册为buffer
...
class QAdaptiveAvgPooling2d(QModule):
    def __init__(self, output_size=(1, 1), qi=False, num_bits=None):
